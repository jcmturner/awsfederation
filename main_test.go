...
func TestFederationUser(t *testing.T) {
	...
	var tests = []struct {
		...
		{"POST", "/" + httphandling.APIVersion + "/federationuser", "{\"Arn\":\"" + Test_Arn + "\",\"Credentials\":{\"SecretAccessKey\":\"REDACTED\",\"SessionToken\":\"\",\"Expiration\":\"" + Test_Expiration + "\",\"AccessKeyId\":\"" + Test_AccessKeyId + "\"},\"TTL\":0,\"MFASerialNumber\":\"\",\"MFASecret\":\"\"}", http.StatusBadRequest, "{\"Message\":\"Federation user not already exists.\",\"HTTPCode\":400,\"ApplicationCode\":103}"},
